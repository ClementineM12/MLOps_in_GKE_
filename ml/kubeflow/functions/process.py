...
def process_metadata(
    ...
    Args:
        ...
        processed_data_path (str): The folder path in the bucket where all processed data should be uploaded.
    ...
    skin_meta['image'] = skin_meta['image_id'].apply(
        lambda img_id: convert_image_into_array(img_id, bucket_name=bucket, images_dir=images_dir, data_path=data_path)
    )
...
def create_segmented_pictures(
    ...
    Args:
        ...
        metadata (pd.DataFrame): DataFrame containing image metadata with an 'image_id' column.
